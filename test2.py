...
def analyze_java_file(file_path):
    ...
    try:
        ...
        for path, node in tree:
            ...
            # Exit scope as needed - safely check for position attributes
            # Note: Removed the problematic Position.end attribute check
            if len(current_scope) > 0 and len(path) > 0:
                # Use a simpler heuristic for exiting scope - if we're leaving a block
                if path[-1] != current_scope[-1]:
                    # This is a different path than our current scope, so we might need to pop
                    try:
                        # Try to use position if available, but don't rely on specific attributes
                        if hasattr(current_scope[-1], 'position') and hasattr(node, 'position'):
                            # Check if we're outside the current scope somehow
                            current_scope.pop()
                    except (AttributeError, TypeError):
                        # If there's any issue with positions, just continue
                        pass
            ...
            if isinstance(node, javalang.tree.LocalVariableDeclaration):
                try:
                    ...
                    # Skip if there's an issue with this declaration
                    pass
            ...
            # Exit nesting stack when appropriate - Modified to avoid Position.end issues
            if nesting_stack and isinstance(nesting_stack[-1], str):  # Only handle string markers
                # If we're at a closing brace or leaving a block, pop markers
                if isinstance(node, javalang.tree.BlockStatement) and hasattr(node, 'position'):
                    # Simple heuristic: Pop one level of nesting when we exit a block
                    if path and path[-1] != nesting_stack[-1]:
                        nesting_stack.pop()
...
def analyze_java_directory(directory_path):
    ...
    Uses sequential processing instead of parallel processing.
    """
...
def main(directory_path, output_file=None):
    """
    Main function to run the analysis and save results.
    
    Args:
        directory_path: Path to directory containing Java files
        output_file: Optional path for the output file (default: java_feature_extraction.xlsx)
    """
    if not output_file:
        output_file = os.path.join(directory_path, 'java_feature_extraction.xlsx')
    
    print(f"Starting analysis of Java files in: {directory_path}")
    
    # Analyze directory
    results_df = analyze_java_directory(directory_path)
...
output_file = "java_feature_extraction.xlsx"  # Will be saved in the directory_path

# Call the main function directly
df = main(default_dir, output_file=output_file)
print(df)