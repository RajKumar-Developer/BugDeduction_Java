...
def draw_graph(graph):
    fig, ax = plt.subplots(figsize=(30, 8))  # Explicitly create figure and axis
    pos = nx.nx_agraph.graphviz_layout(graph, prog='dot')  # Use Graphviz layout
    labels = nx.get_node_attributes(graph, 'label')
    nx.draw(graph, pos, labels=labels, with_labels=True, node_size=2000, node_color='lightblue', 
            font_size=10, font_color='black', font_weight='bold', arrows=False, ax=ax)
    plt.show()
